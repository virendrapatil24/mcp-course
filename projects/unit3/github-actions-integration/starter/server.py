...
async def get_workflow_status(workflow_name: Optional[str] = None) -> str:
    ...
    worklow_events = [e for e in events if e.get("workflow_run") is not None]

    if workflow_name:
        worklow_events = [
            e for e in events if e["workflow_run"].get("name") == workflow_name
        ]

    workflows = {}
    for event in worklow_events:
